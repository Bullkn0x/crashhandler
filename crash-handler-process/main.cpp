	...
	while (!exitApp) {
		...
		if (!alive) {
			...
			if (!processes.at(index)->getCritical()) {
				int code = MessageBox(
					NULL,
					"An error occurred which has caused Vurteau Broadcasting Studio to close. Don't worry! If you were streaming or recording, that is still happening in the background."
